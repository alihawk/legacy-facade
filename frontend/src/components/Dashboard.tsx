...
export default function Dashboard({ resources, customization, isSpooky = false }: DashboardProps) {
  ...
  return (
    <div className="space-y-8 animate-emerge">
      <div className={`relative overflow-hidden rounded-2xl p-8 text-white ${isSpooky ? 'bg-gradient-to-br from-slate-900 via-cyan-900/50 to-slate-800 border border-cyan-500/40 shadow-lg shadow-cyan-500/20' : 'bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500'}`}>
        ...
        <div className={`absolute top-1/2 right-1/4 w-32 h-32 rounded-full blur-2xl pointer-events-none ${isSpooky ? 'bg-cyan-400/30' : 'bg-purple-400/20'}`} />
      ...
      {/* Stats Grid - Clean hover effects - Conditional based on customization */}
      {customization?.dashboard?.statsCards !== false && (
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-4xl font-bold ${isSpooky ? 'text-cyan-400' : 'bg-gradient-to-r from-indigo-600 to-indigo-400 bg-clip-text text-transparent'}`}>
                  {resources.length}
                </p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-4xl font-bold ${isSpooky ? 'text-cyan-400' : 'bg-gradient-to-r from-teal-600 to-emerald-400 bg-clip-text text-transparent'}`}>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-4xl font-bold ${isSpooky ? 'text-cyan-400' : 'bg-gradient-to-r from-amber-600 to-amber-400 bg-clip-text text-transparent'}`}>
                  {totalFields}
                </p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-4xl font-bold ${isSpooky ? 'text-cyan-400' : 'bg-gradient-to-r from-rose-600 to-rose-400 bg-clip-text text-transparent'}`}>
                  100%
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Resource Distribution Chart - Conditional */}
        {customization?.dashboard?.barChart !== false && (
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={chartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke={isSpooky ? '#1e293b' : '#f0f0f0'} />
                  <XAxis 
                    dataKey="name" 
                    tick={{ fill: isSpooky ? '#22d3ee' : '#6b7280', fontSize: 12 }}
                    angle={-45}
                    textAnchor="end"
                    height={80}
                  />
                  <YAxis tick={{ fill: isSpooky ? '#22d3ee' : '#6b7280', fontSize: 12 }} />
                  <Tooltip 
                    contentStyle={{ 
                  ...
                  <Bar 
                    dataKey="count" 
                    fill={isSpooky ? "url(#colorGradientSpooky)" : "url(#colorGradient)"} 
                    radius={[8, 8, 0, 0]}
                  />
                  <defs>
                    ...
                    <linearGradient id="colorGradientSpooky" x1="0" y1="0" x2="0" y2="1">
        ...
        {customization?.dashboard?.recentActivity !== false && (
